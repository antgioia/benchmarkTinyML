...
low_rank_data = load_csv_files_from_folder('energyConsumed/lowRankApproximation/vgg16')
pre_data = load_csv_files_from_folder('energyConsumed/pre/vgg16')
quantization_data = load_csv_files_from_folder('energyConsumed/quantization/vgg16')
pruning_casuale_non_strutturato_data = load_csv_files_from_folder('energyConsumed/pruning/vgg16/pruned_casuale_non_strutturato')
pruning_global_data = load_csv_files_from_folder('energyConsumed/pruning/vgg16/pruned_global')
pruning_non_strutturato_data = load_csv_files_from_folder('energyConsumed/pruning/vgg16/pruned_non_strutturato')
pruning_strutturato_per_canali_data = load_csv_files_from_folder('energyConsumed/pruning/vgg16/pruned_strutturato_per_canali')
...
if 'energy_consumed' in low_rank_data.columns and 'energy_consumed' in pre_data.columns and 'energy_consumed' in quantization_data.columns:
    # Estrazione dell'energia consumata
    ...
    print(f"Mann-Whitney U (Low Rank vs Pre): {t_stat_lr_pre:.3f}, P-value: {p_value_lr_pre:.3f}")
    t_stat_quantization_pre, p_value_quantization_pre = stats.mannwhitneyu(quantization_energy_consumed, pre_energy_consumed)
    print(f"Mann-Whitney U (Quantization vs Pre): {t_stat_quantization_pre:.3f}, P-value: {p_value_quantization_pre:.3f}")
    ...
    print(f"Mann-Whitney U (Pruning Casuale vs Pre): {t_stat_pruning_pre:.3f}, P-value: {p_value_pruning_pre:.3f}")
    t_stat_pruning_global_pre, p_value_pruning_global_pre = stats.ttest_ind(pruning_global_energy_consumed, pre_energy_consumed)
    ...
    print(f"Mann-Whitney U (Pruning Non Strutturato vs Pre): {t_stat_pruning_non_strutturato_pre:.3f}, P-value: {p_value_pruning_non_strutturato_pre:.3f}")
    ...
    print(f"Mann-Whitney U (Pruning Strutturato vs Pre): {t_stat_pruning_strutturato_pre:.3f}, P-value: {p_value_pruning_strutturato_pre:.3f}")
    ...
    plt.title("Comparison of vgg16 energy_consumed between Pre, Quantization and Low-Rank Approximation")
    plt.xticks(rotation=45)
    plt.savefig("energyConsumedAnalysis/result/pre_quantization_low_rank_approximation_vgg16.png")
    plt.show()
    plt.figure(figsize=(12, 8))
    sns.boxplot(x='Method', y='energy_consumed', data=boxplot_data2)
    plt.ylabel('energy_consumed')
    plt.title('Comparison of vgg16 energy_consumed between Pre and All Techniques of Pruning')
    plt.xticks(rotation=45)
    plt.savefig("energyConsumedAnalysis/result/pre_pruning_vgg16.png")
    plt.show()
else:
