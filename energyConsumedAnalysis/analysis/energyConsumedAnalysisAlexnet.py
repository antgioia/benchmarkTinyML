...
low_rank_data = load_csv_files_from_folder('energyConsumed/lowRankApproximation/alexnet')
pre_data = load_csv_files_from_folder('energyConsumed/pre/alexnet')
quantization_data = load_csv_files_from_folder('energyConsumed/quantization/alexnet')
pruning_casuale_non_strutturato_data = load_csv_files_from_folder('energyConsumed/pruning/alexnet/pruned_casuale_non_strutturato')
pruning_global_data = load_csv_files_from_folder('energyConsumed/pruning/alexnet/pruned_global')
pruning_non_strutturato_data = load_csv_files_from_folder('energyConsumed/pruning/alexnet/pruned_non_strutturato')
pruning_strutturato_per_canali_data = load_csv_files_from_folder('energyConsumed/pruning/alexnet/pruned_strutturato_per_canali')
...
if 'energy_consumed' in low_rank_data.columns and 'energy_consumed' in pre_data.columns and 'energy_consumed' in quantization_data.columns:
    ...
    p_value_pruning_g <= alpha and p_value_pruning_ns <= alpha and p_value_pruning_spc <= alpha:
        ...
        plt.title("Comparison of alexnet energy_consumed between Pre, Quantization and Low-Rank Approximation")
        plt.xticks(rotation=45)
        plt.savefig("energyConsumedAnalysis/result/pre_quantization_low_rank_approximation_alexnet.png")
        plt.show()
        plt.figure(figsize=(12, 8))
        sns.boxplot(x='Method', y='energy_consumed', data=boxplot_data2)
        plt.ylabel('energy_consumed')
        plt.title('Comparison of alexnet energy_consumed between Pre and All Techniques of Pruning')
        plt.xticks(rotation=45)
        plt.savefig("energyConsumedAnalysis/result/pre_pruning_alexnet.png")
        plt.show()
else:
