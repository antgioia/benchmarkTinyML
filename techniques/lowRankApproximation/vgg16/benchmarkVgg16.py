...
# Classe per VGG16 con approssimazione a basso rango
...
model = LowRankVGG16()
model.load_state_dict(torch.load('minimizedModels/lowRankApproximation/vgg16_low_rank.pth'), strict=False)
...
model_size = os.path.getsize("minimizedModels/lowRankApproximation/vgg16_low_rank.pth") / (1024 ** 2)

# Salva i risultati in un file di testo
save_results("benchmark/lowRankApproximation/vgg16", "vgg16", accuracy, precision, recall, auc_roc, inference_time, throughput, model_size)
