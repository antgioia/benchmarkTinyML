...
quantized_model.load_state_dict(torch.load("minimizedModels/quantization/vgg16_dynamic_quantized.pth"))
...
val_dataset_vgg16 = datasets.ImageFolder(root='dataset/imagenet-mini/train', transform=transform)
val_loader_vgg16 = DataLoader(dataset=val_dataset_vgg16, batch_size=128, shuffle=False)
...
accuracy, precision, recall, auc_roc, inference_time, throughput =  evaluate_model(quantized_model, val_loader_vgg16, device)

# Calcola la dimensione del modello
model_size = os.path.getsize("minimizedModels/quantization/vgg16_dynamic_quantized.pth") / (1024 ** 2)
...
save_results("benchmark/quantization/vgg16", "vgg16", accuracy, precision, recall, auc_roc, inference_time, throughput, model_size)
