...
val_dataset_resnet18 = datasets.ImageFolder(root='dataset/imagenet-mini/train', transform=transform)
val_loader_resnet18 = DataLoader(dataset=val_dataset_resnet18, batch_size=256, shuffle=False)
...
accuracy, precision, recall, auc_roc, inference_time, throughput =  evaluate_model(quantized_model, val_loader_resnet18, device)

# Calcola la dimensione del modello
model_size = os.path.getsize("minimizedModels/quantization/resnet18_dynamic_quantized.pth") / (1024 ** 2)
...
save_results("benchmark/quantization/resnet18", "resnet18", accuracy, precision, recall, auc_roc, inference_time, throughput, model_size)
